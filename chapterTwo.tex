...
%	This chapter will contain some source code. It will be pulled in
%	from a file in the same directory as the this file (and the rest of
%	the document source files, too)

%	The following code is adapted from the code listed at the following
%	url:
...
%	The following command tells the LaTeX to format the supplied source
%	file (which usually follows the use of the command) using the supplied
%	options.
%	In this instance, we're telling LaTeX to format the listing with syntax
%	highlighting for C++, placing line numbers at the left, and stepping
%	numbers along by 1 (1, 2, 3, 4, ...n)
\lstset{numbers=left, stepnumber=1, language=C++}

%	The following command tells LaTeX to insert the contents of a supplied
%	source file, but as a file contents listing. This is different to how a
%	standard \insert command, because the contents are not taken as
%	valid LaTeX code.
...
%	The following code tells LaTeX to ignore the commands between the
%	\begin{} and \end{} commands. They will be outputted as a listing, using
%	whatever formatting has already been supplied.
	\begin{lstlisting}
		\usepackage{listings}
	\end{lstlisting}
